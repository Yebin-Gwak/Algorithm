...
public class Main {
	static class Mal{
		...
		public Mal(int num, int x, int y, int d) {
			this.num = num;
			this.x = x;
			this.y = y;
			this.d = d;
	...
	private static void play() {
		...
		while(++turn <= 1000) {
			for(Mal mal : mList) {
				...
				// 다음 땅 물색. 다음 땅이 2라면 방향 미리 바꿔줌, 반대 방향도 2라면 이동 스킵
				...
				int mapSize = mMap[x].get(y).size();
				
				// 태우기
				for(int i = index; i < mapSize; i++) {
					deque.addFirst(mMap[x].get(y).pollLast());
				}
				
				
				// 다음 땅 이동. 다음 땅이 2라면 이동 안함. 1이라면 pollLast로 넣기, 0이면 정방향
				x += dx[mal.d];
				y += dy[mal.d];
				int color = map[x][y];
				if(color == 1) {
